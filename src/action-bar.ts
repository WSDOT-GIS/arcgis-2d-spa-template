...
/**
 * Finds the related Panel element for a given HTMLCalciteActionElement element based on the
 * "data-toggles" attribute of the HTMLCalciteActionElement.
 *
 * @param action - A calcite-action element
 * @returns
 */
const findRelatedPanel = <T extends HTMLElement>(action: HTMLCalciteActionElement): T | null => {
  const panelId = action.dataset.toggles;
  if (!panelId) {
    console.warn("HTMLCalciteActionElement does not have a data-toggles attribute:", action);
    return null;
  }
...
function* iterateActionsAndPanels(actionBar: HTMLCalciteActionBarElement) {
  ...
  if (activeActions) {
    for (const action of activeActions) {
      ...
      if (!panel) {
        /* __PURE__ */ console.warn("Could not find related panel", { action, panel });
        continue;
      }
      yield { action, panel };
    }
...
/**
 * Event handler that toggles "collapsed" state of the panel related to the HTMLCalciteActionElement
 * element that was clicked. The relationship is determined by the "data-toggles" attribute on the
 * HTMLCalciteActionElement element, which should contain the id of the related Panel element. Also
 * sets the HTMLCalciteActionElement's "active" state to reflect the panel's collapsed state.
 *
...
const togglePanel = (event: PointerEvent) => {
   ...
   *
   * The panel itself wasn't actually clicked. Just wanted to keep the variable name short.
   */
  const clickedPanel = findRelatedPanel<HTMLCalcitePanelElement | HTMLCalciteBlockElement>(
    clickedAction,
  );
  if (!clickedPanel) {
  ...
  for (const { action: currentAction, panel: currentPanel } of iterateActionsAndPanels(
    toolsActionBar,
  )) {
  ...
  const shellPanel = clickedPanel.parentElement as HTMLCalciteShellPanelElement | null;
  if (shellPanel) {
...
/**
 * Sets up the click event listeners for all HTMLCalciteActionElement elements that have a
 * "data-toggles" attribute, which indicates they are related to a Panel element.
 *
 * @param actionBar
 */
